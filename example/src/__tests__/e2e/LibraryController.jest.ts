import { BookEntity, LibraryEntity, LibraryShelfEntity } from "../../entities"
...
describe('LibraryController Endpoint', () => {
  ...
  it('Can update items from CrudController', async () => {
    ...
    expect(afterCreated.data.shleves.map((o) => o.isbn)).toEqual(shelves.map((o) => o.slug))
  })

  it.only('Can create nested item in CrudController', async () => {
    // Create new one
    ...
    expect(afterCreated.httpStatusCode).toEqual(200)
    expect(afterCreated.data.slug).toEqual(targetSlug)
    expect(afterCreated.data.shelves.map((o) => o.isbn).sort()).toEqual(shelves.map((o) => o.slug).sort())
  })
